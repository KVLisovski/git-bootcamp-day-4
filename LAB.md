...
В HEAD зашел рекомендованным способом самым простым `git switch --detach HEAD~2`. Сделано коммит с экспериментальными гипотезами. Что то типа : `git commit -m "exp: ещё одна гипотеза"` Создание ветки спасает и это действительно просто. НА счет reflog и чистки через 90 дней ... в целом это спасение, но мне кажется если работать скаажем в IDE тяжело попасть в такое. А так reflog спасение, просто зная хэш и вуаля коммит спасен. В голову приходит ... не коммитить не посмотреат status 
